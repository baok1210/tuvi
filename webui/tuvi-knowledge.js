...
const PATTERNS_DATA = [
  ...
  {ten:"Tam Hóa Gia Hội", check: (data) => { let c=0; data.thap_nhi_cung[0].chinh_tinh.concat(data.thap_nhi_cung[0].phu_tinh).concat(data.thap_nhi_cung[0].sat_tinh).forEach(s=>{if(s.includes("(Hóa Lộc)")||s.includes("(Hóa Quyền)")||s.includes("(Hóa Khoa)"))c++}); return c>=3; }, y_nghia:"Phú quý song toàn hiếm có.", source:"紫微斗数全书·三奇加会"},
  ...
  {ten:"Song Lộc Triều Viên", check: (data) => { const m=data.thap_nhi_cung[0]; return m.phu_tinh.includes("Lộc Tồn")&&m.chinh_tinh.some(s=>s.includes("(Hóa Lộc)")); }, y_nghia:"Đại phú, giàu có bậc nhất."},
  ...
  {ten:"Dương Đà Giáp Kỵ", check: (data) => { for(let p=0;p<12;p++){if(gC(data,(p-1+12)%12).sat_tinh.includes("Kình Dương")&&gC(data,(p+1)%12).sat_tinh.includes("Đà La")&&(gC(data,p).chinh_tinh.some(s=>s.includes("(Hóa Kỵ)"))||gC(data,p).phu_tinh.some(s=>s.includes("(Hóa Kỵ)"))))return true}return false; }, y_nghia:"Đại hung. Cẩn thận huyết quang, phẫu thuật, kiện tụng.", source:"紫微斗数骨髓赋·羊陀夹忌"},
  ...
  {ten:"Hóa Lộc Nhập Mệnh", check: (data) => data.thap_nhi_cung[0].chinh_tinh.some(s=>s.includes("(Hóa Lộc)")), y_nghia:"Sinh tài thuận lợi, nhân duyên tốt, cơ duyên nhiều.", source:"紫微斗数全书·四化论"},
  {ten:"Hóa Kỵ Nhập Mệnh", check: (data) => data.thap_nhi_cung[0].chinh_tinh.some(s=>s.includes("(Hóa Kỵ)")), y_nghia:"Cố chấp, tâm lý chướng ngại, hoặc tiềm ẩn vấn đề sức khỏe.", source:"紫微斗数全书·四化论"},
  ...
  {ten:"Hóa Lộc Nhập Tài", check: (data) => data.thap_nhi_cung[4].chinh_tinh.some(s=>s.includes("(Hóa Lộc)")), y_nghia:"Tài lộc dồi dào, kiếm tiền thuận lợi.", source:"紫微斗数全书·四化论"},
  {ten:"Hóa Quyền Nhập Quan", check: (data) => data.thap_nhi_cung[8].chinh_tinh.some(s=>s.includes("(Hóa Quyền)")), y_nghia:"Quyền lực, thăng tiến trong sự nghiệp.", source:"紫微斗数全书·四化论"},
  {ten:"Hóa Khoa Nhập Mệnh", check: (data) => data.thap_nhi_cung[0].chinh_tinh.some(s=>s.includes("(Hóa Khoa)")), y_nghia:"Văn tài, học thức, danh tiếng.", source:"紫微斗数全书·四化论"},
  ...
  {ten:"Hóa Kỵ Nhập Tài", check: (data) => data.thap_nhi_cung[4].chinh_tinh.some(s=>s.includes("(Hóa Kỵ)")), y_nghia:"Tài chính có vấn đề, cẩn thận chi tiêu."},
];
