...
def khoangCachCung(cung1, cung2, chieu=1):
    if chieu == 1:  # Con trai, chiều dương
        if cung2 >= cung1:
            ...
            return 12 + cung2 - cung1
    else:
        if cung1 >= cung2:
            return cung1 - cung2
        else:
            return 12 + cung1 - cung2
...
def timTuVi(cuc, ngaySinhAmLich):
    ...
    Args:
        cuc (int): Số cục (2=Thủy, 3=Mộc, 4=Kim, 5=Thổ, 6=Hỏa)
        ngaySinhAmLich (int): Ngày sinh âm lịch (1-30)

    Returns:
        int: Vị trí Tử Vi (1=Tý, 2=Sửu, ..., 12=Hợi)
    """
